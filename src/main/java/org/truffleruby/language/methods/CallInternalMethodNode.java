...
public abstract class CallInternalMethodNode extends RubyBaseNode {
    ...
    private void logCalls(InternalMethod method, DispatchNode dispatchNode, RubyClass metaclass, CallTarget currentCallTarget) {
        if (getContext().monitorCalls) {
            ...
            getContext().logger.info(getContext().stage + "\t" + method.getName() + "\t" + (metaclass == null ? "NA" : metaclass.getMetaSimpleName()) + "\t" + getSourceSectionAbbrv(dispatchNode) + "\t" + (dispatchNode == null ? -666 : dispatchNode.getSplitID()) + "\t" + method.isBuiltIn() + "\t" + method.getDeclaringModule().getName()+"\t"+ ((dispatchNode == null || dispatchNode.getRubyCallNode() == null) ? "NA" : dispatchNode.getRubyCallNode().getCallSiteID()));
        }
    }
